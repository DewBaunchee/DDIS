...
class ClientForm : VBox() {
    ...
    private val disabilityRepository = provide(DisabilityRepository::class.java)
    ...
    init {
        ...
        hbox {
            ...
            hbox {
                ...
                button("New Deposit") {
                    onAction = EventHandler {
                        depositModeService.newDeposit(model.item.id)
                    }
                }
            }
            hbox {
        ...
        hbox {
            ...
            labeledField("Mobile Phone Number") {
                textfield(model.mobilePhoneNumber) {
                    val regex = Regex(
                        "\\s*\\+375\\s*-?\\s*\\(?((25)|(29)|(33)|(44))\\)?\\s*-?\\s*[0-9]{3}\\s*-?\\s*[0-9]{4}"
                    )
                    promptText = "+375-(25|29|33|44)-XXX-XXXX"
                    validator(
                        ValidationTrigger.OnBlur
                    ) {
                        if (!it.isNullOrBlank() && !regex.matches(it))
